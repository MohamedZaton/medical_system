/// website links
const String kMAUrl = "http://www.macomp.com/";
const String kEduBuyUrl = "http://www.edubuyers.com/";
const String kRegisterTxt = "Register";
const String kRegToWinTxt = "Register To Win";
const String kSubmitTxt = "Submit";
const String kSendTxt = "Send";
const String kFullNameTxt = "Full Name";
const String kSchoolTxt = "School District";
const String kTitleTxt = "Title";
const String kPortfolioTxt = "Portfolio";
const String kTeckStack = "Tech Stack :";
const String kNameTxt = "الاسم";
const String kAddressTxt = "العنوان";
const String kAdsSpaceTxt = "مساحه إعلانية";

const String kPhoneNumberTxt = "رقم الهاتف";
const String kNameDemo = "محمد عبد الله عارف";
const String kPhoneDemo = "01221123345";

const String kPassword = "كلمة المرور";
const String kSignInText = "تسجيل الدخول";
const String kDoctorPaperText = "صورة الروشتة";
const String kDoctorBk = "حجز دكتور";
const String kLabsBk = "حجز المعمل ";
const String kSelectDay = "اختيار اليوم ";
const String kSelectTime = "اختيار الوقت ";
const String kSendPaperText = "ارسال الروشتة";
const String kSignUpText = "تسجيل حساب";
const String kSignUpSuccessText = "تم تسجيلك بنجاح ";
const String kSignUpFailedText = "هناك خطافى التسجيل";
const String kSignOutText = "تسجيل الخروج";
const String kCompleted = "مكتمل";
const String kWaiting = "قيد الانتظار";
const String kCancel = "تم الالغاء";
const String kDelivery = "فى الطريق";
const String kOrderYetTxt = "لم تقم باى طلب حتى الان ";
const String kNoNetworkTxt = "لا يوجد اتصال بالإنترنت ";
const String kNoParentDepTxt = "لا يوجد اقسام ";
// setting
const kProfileTxt = "الملف الشخصي";
const kLanguageTxt = "اللغة";
const kPriceTxt = "السعر";
const kEgpTxt = "جنية";
const kDepartmentsTxt = "الاقسام";
const kServiceProviderTxt = "مقدم الخدمة";
const kWhatsappTxt = "الواتساب ";
const kFacebookTxt = "فيس بوك ";
const kContactUsTxt = "تواصل معنا";
const kShareAppTxt = "مشاركة التطبيق";
const kRateAppTxt = "تقيم التطبيق";
const kAboutAppTxt = "حول التطبيق";
const kSelectedAddressTxt = "اختيار العنوان";
const kDemoAddressOneTxt = "منطقة العوام ";
const kDemoAddressTwoTxt = "علم الروم ";
const kDemoAddressThreeTxt = "شارع الاسكندرية";
const kDemoAddressFourTxt = "مطروع العوام عمارة ٢";
const String kEmptyTxt = "لا يوجد ";
const String kAnalysesTitleTxt = "انواع المحاليل ";
const String kAnalysesTotalTxt = "مجموع التحليل  ";
const String kUploadDoctorPaperTxt = "صورة الروشته";
const String kClinicExaminationTxt = "كشف عيادة";
const String kHomeExaminationTxt = "كشف منزلى";
const String kChooseTypeTestsTxt = "اختيار نوع التحليل";
const String kChooseTypeBookingTxt = "اختيار الطريقة الحجز ";
const String kUploadFaildTxt = "حدث خطا ما يرجى المحاول مره اخر ";
const String kUploadSuccessTxt = "تم ارسال طلبك  ";
const String kCancelTitleTxt = "طلب الالغاء  ";
const String kCancelRejectedTxt = "لم يتم طلب الالغاء ";
const String kCancelAcceptedTxt = "تم طلب الالغاء ";

//"طلب الالغاء", "لم يتم طلب الالغاء ",
const String kBackTxt = "رجوع";
const String kTryAgainTxt = "أعد المحاولة ";
const String kSectionName = "section";
const String kLocalPersonsDB = "PersonDB";
const String kDetailsPort =
    "If you need a Mobile Developer, feel free to contact me!";
const String kRegisterText =
    "Thank you for registering for the drawing from M & A’s booth 1334 at TCEA. To complete  your registration, please like us on LinkedIn \nfollow the link below,";
